...
class User(Base, TimestampMixin):
    ...
    role_id: Mapped[str | None] = mapped_column(String, ForeignKey("roles.id"), nullable=True, index=True)
...
class Reminder(Base):
    ...
    trigger_time: Mapped[dt.datetime] = mapped_column(DateTime(timezone=False), nullable=False, index=True)
    status: Mapped[str] = mapped_column(String, nullable=False, default="scheduled", index=True)
    channel: Mapped[str] = mapped_column(String, nullable=False, default="in_app", index=True)
...
class Schedule(Base, TimestampMixin):
    ...
    assigned_to: Mapped[str] = mapped_column(String, nullable=False, index=True)
    start_time: Mapped[dt.datetime] = mapped_column(DateTime(timezone=False), nullable=False, index=True)
    end_time: Mapped[dt.datetime] = mapped_column(DateTime(timezone=False), nullable=False, index=True)
    status: Mapped[str] = mapped_column(String, nullable=False, default="scheduled", index=True)
    source: Mapped[str] = mapped_column(String, nullable=False, default="local")
    synced_to_office365: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    office365_event_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
    recurrence_rule: Mapped[str | None] = mapped_column(String, nullable=True)
    parent_schedule_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
